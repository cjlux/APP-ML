  ...
  \begin{tcolorbox}[title=Beaucoup d'approches possibles pour les algorithmes de {\em Machine Learning}]
    {\small
      \begin{minipage}[t]{.55\textwidth}
        Supervised learning:
        \begin{itemize}
        \item \Chocolate{Neural Networks}
...
\end{tikzpicture}
  ...
  \visible<1->{Un \bfdarkchoco{neurone artificiel}:
    \begin{itemize}
    \item <1-> reçoit les données d'entrée $(x_{i})_{i=1..n}$ affectées des  \textbf{poids} $(w_i)_{i=1..n}$ ({\em weights}) 
    \item <2-> calcule la \textbf{somme pondérée} de ses entrées moins le biais $\sum_i{w_{i}\,x_i} - b$
    \item <3-> produit en sortie une \textbf{activation} $f(\sum_i{w_{i}\,x_i} - b)$, calculée avec une fonction d'activation $f$ (en général non-linéaire).
    \end{itemize}
  }
...
\begin{frame}{}
  ...
  \begin{itemize}
  \item <2-> Le jeu de données est découpé en (mini) {\bf batches} de taille \code{batch\_size}
  \item <3-> Après chaque {\em feed forward} l'algorithme de {\em Back Propagation} modifie les poids
    du réseau de neurones pour minimiser l'erreur $e$.
  \end{itemize}
\end{frame}
%===============================================================================
...
L'entraînement avec le jeu de données complet est répété \code{n\_epoch} fois...
...
\begin{frame}{Mise en oeuvre dans l'APP}
  ...
  \begin{tcolorbox}[title=Étape 2 -- Résolution d'un problème de classification]
    \begin{itemize}
    \item On dispose de données labellisées acquises sur un banc de perçage instrumenté.
    \item Une première étape de pré-traitement permet de calculer des méta-données
      ({\em features}) à partir des données acquises sur le banc.
    \item \Chocolate{L'étude des pré-traitements sera abordée lors de séances de {\em traitement du signal} dédiées.}
    \item Le \textbf{problème} proposé : entraîner un réseau de neurones dense à reconnaître le matériau percé
      à partir des méta-données labellisées extraites du banc de perçage.
    \end{itemize}
